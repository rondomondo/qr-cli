 ...
 *
 * Deps   : qr-code-styling  canvas  sharp
 */
...
async function render(opts) {
  ...
  // node-canvas shim: QRCodeStyling expects a browser Canvas constructor
  // We inject it via the global so the library picks it up
  global.document = {
    createElement: (tag) => {
      if (tag === "canvas") return createCanvas(cfg.width, cfg.height);
      return {};
    },
  };

  await qr.getRawData("png"); // force internal render on node-canvas

  // Grab the canvas that qr-code-styling used
  const canvas = qr._canvas;
  if (!canvas) die("Render failed: no canvas produced");

  const border = parseInt(opts.border || 0, 10);
  let rawPng = canvas.toBuffer("image/png");
